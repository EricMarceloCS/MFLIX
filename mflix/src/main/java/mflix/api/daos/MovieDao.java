...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
    	...
    	Document doc = null;
    	try {
    		doc = moviesCollection.find(new Document("_id", new ObjectId(movieId))).first();
    	} catch (Exception e ) {
    		e.getMessage();
    	}
    ...
    public Document getMovie(String movieId) {
        ...
        //lookup variables
        Variable<String> id = new Variable<String>("id", "$_id");
        ...
        Bson expr = new Document("$expr", eq);
        Bson cMatch = new Document("$match", expr);
        
        List<Bson> lPipeline = new ArrayList<>();
        lPipeline.add(cMatch);
        lPipeline.add(sort);
        
        Bson lookup = Aggregates.lookup("comments", let, lPipeline, "comments");  
    ...
    private Bson buildFacetStage() {
        ...
        return Aggregates.facet(
                ...
                new Facet("movies", Aggregates.addFields(new Field<Object>("title", "$title"))));
    }
